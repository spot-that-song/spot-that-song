<template>
  <v-container>
    ...
    <div
      v-if="resultsStage"
    >You got {{correct}} right out of {{questions.length}} questions. Your percentage is {{perc}}%. <a @click.prevent="reset">Click here</a> to go back.</div>
  </v-container>
</template>
...
export default {
  ...
  created() {
    arr.forEach(item => {
      ...
      let switchIndex = Math.round(Math.random() * 2);
